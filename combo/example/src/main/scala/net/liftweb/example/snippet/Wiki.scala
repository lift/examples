...
// show determines which one is used. bind hooks the content into the lift view
      ...
      class Wiki extends MetaWikiEntry {
        ...
        def choosebind(xhtml: NodeSeq) = {
          def pageName =
            S.param("wiki_page") openOr "HomePage" // set the name of the page
          ...
          // find the entry in the database or create a new one
          def entry =
            find(By(WikiEntry.name, pageName)) openOr create.name(pageName)

          // is it a new entry?
          def isNew = !entry.saved_?
          def toEdit =
            isNew || (S.param("param1").map(_ == "edit") openOr false)
        ...
        private def editEntry(entry: WikiEntry,
                              isNew: Boolean,
                              pageName: String) = {
          ...
          val submitButton =
            SHtml.submit(isNew ? "Add" | "Edit", () => entry.save)
